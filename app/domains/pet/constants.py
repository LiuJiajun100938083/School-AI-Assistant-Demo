...
TEACHER_COIN_SOURCES = {
    ...
    "upload_knowledge_doc":  {"amount": 8,  "label": "上传知识库文档"},
    "upload_attachment":     {"amount": 3,  "label": "上传作业附件"},
    "upload_board_file":     {"amount": 3,  "label": "上传佈告板文件"},
    "create_assignment":     {"amount": 5,  "label": "创建作业"},
    "create_dictation":      {"amount": 5,  "label": "创建默写"},
    "social_like_given":     {"amount": 1,  "label": "给别人点赞"},
    "social_like_received":  {"amount": 2,  "label": "收到点赞"},
    "pet_chat_reward":       {"amount": 2,  "label": "宠物聊天"},
}
...
# 管理员金币无限（不扣余额）— 已关闭，所有角色统一扣币
ADMIN_UNLIMITED_COINS = False
...
def build_pet_chat_prompt(pet_name: str, personality: str,
    ...
    return (
        ...
        f"你的小伙伴是一名中学生，请用简短、可爱的语气回复。\n"
        f"每次回复控制在 2-3 句话以内（不超过 80 字）。\n"
        f"你可以鼓励小伙伴学习，回答学习问题，或者撒娇卖萌。\n"
        f"绝对不要回答涉及暴力、色情、政治等不当内容的问题，"
        f"遇到这类问题请可爱地拒绝。\n"
        f"请用繁体中文或简体中文回复（跟随小伙伴的语言）。"
    )

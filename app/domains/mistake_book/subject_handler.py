...
只輸出 JSON。"""
    ...
    def build_practice_grading_prompt(
        ...
        # Auto-detect English if question is in English
        if lang == "zh" and self._is_english_text(question_text):
            lang = "en"

        if lang == "en":
            return f"""You are an experienced {self.display_name} teacher. Please grade the following practice question.
...
## Grading Levels (strictly follow boundaries)
| Level | Meaning | is_correct |
|-------|---------|------------|
| A | Completely correct, both result and expression are valid | true |
| B | Essentially correct, minor expression issues (not simplified, formatting) | true |
| C | Main approach correct, but answer incomplete/partially missing | false |
| D | Clear method or calculation errors, but shows partial understanding | false |
| E | Fundamentally misunderstands core concepts | false |
| F | Irrelevant, blank, refused, or severely off-topic | false |

## error_type enum (choose exactly one, use null for A level)
careless / concept / calculation / method / format / incomplete / irrelevant

## Output (JSON only, no explanation)
```json
{{"correctness_level": "A", "is_correct": true, "error_analysis": "1-2 sentence feedback in English", "error_type": null}}
```"""

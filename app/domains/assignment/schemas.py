...
class CreateAssignmentRequest(BaseModel):
    ...
    questions: List["QuestionInput"] = Field(default=[], description="Form 類型題目列表")

    @validator("assignment_type")
    def validate_assignment_type(cls, v):
        if v not in AssignmentType.ALL:
            raise ValueError(f"assignment_type 必須是 {AssignmentType.ALL} 之一")
        return v

    @validator("questions")
    def validate_questions_for_form(cls, v, values):
        if values.get("assignment_type") == AssignmentType.FORM and len(v) < 1:
            raise ValueError("Form 類型作業至少需要 1 道題目")
        return v
...
class UpdateAssignmentRequest(BaseModel):
    ...
    questions: Optional[List["QuestionInput"]] = Field(default=None, description="Form 題目列表")

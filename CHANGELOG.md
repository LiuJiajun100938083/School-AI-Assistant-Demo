...
- **遊戲中心設計系統** — 新增 `--type-display` ~ `--type-micro` 6 級字體層級 Token 和 `--space-10/12/16` 擴展間距（與登入頁統一）
- **遊戲中心字體層級** — 22 個選擇器的 font-size 從硬編碼 px 替換為設計 Token，建立系統化字體映射
- **遊戲中心 8px 網格** — 修正 `--space-5: 20px` 等非 8px 網格間距，操作按鈕 `gap: 4px` / `padding: 6px 14px` 改為 Token
- **分享彈窗 Token 化** — 所有硬編碼 px 值（`20px 24px`, `16px 24px 24px` 等）替換為設計 Token
- **響應式斷點** — 768px/480px 斷點中的 font-size 改用 Token

### 修改

- **主頁卡片放大** — 適配 iPad 閱讀：圖標 36→48px、標題 13→16px、描述 12→14px、卡片內距 16→20px、圓角 8→12px
- **主頁卡片網格** — 最小寬度 240→280px、間距 12→16px
- **啟動動畫加速** — 圖標/標題/副標題從逐一模糊淡入改為全部同時出現，總時長 3.7s → 2.0s（主頁和遊戲中心）

---

## [v3.0.39] [2026-03-05] 將 SwiftUI 代碼學堂加入遊戲中心 ICT 分類

### 新增

- **遊戲中心 ICT 分類** — 將 SwiftUI 代碼學堂註冊到 `GameConfig.games.ict`，遊戲中心可見並可遊玩

---

## [v3.0.38] [2026-03-05] 登入頁 UI 系統化重構 — 8px 網格 + 字體層級 + 透明度系統

### 重構

- **設計 Token** — 新增 `--type-display` ~ `--type-micro` 6 級字體層級、`--white-primary` ~ `--white-muted` 4 級透明度系統、`--space-16` 擴展間距
- **左側品牌面板** — 吉祥物 320→280px、所有文字改用字體/透明度 Token、間距修正到 8px 網格
- **右側登入面板** — 標題 24→28px、輸入框/按鈕 padding 改用 Token、Logo 改用 CSS class
- **響應式** — iPad 橫屏吉祥物 320→160px

---

## [v3.0.37] [2026-03-05] 登入頁增加動態 QR 碼、字體統一 Noto Sans TC、放大 Logo

### 修改

- **字體統一** — 殘餘 LXGW WenKai TC 引用改為 Noto Sans TC（`.qr-label-zh`、`.brand-quote .quote-chinese`）
- **Logo 放大** — 左側吉祥物 160→320px、右側校徽 48→96px
- **QR 標籤** — 「掃碼手機登入」改為「掃碼登入」

---

## [v3.0.36] [2026-03-04] 修復 QR 碼依賴缺失 + 點名上傳 class_number 類型錯誤

### 修復

- **requirements.txt** — 添加 `qrcode[pil]==8.0` 依賴，修復伺服器缺少 qrcode 模組導致 QR 碼生成 500 錯誤
- **AttendanceService.import_students** — `class_number` 被 `or ""` 誤轉為空字串，MySQL INT 欄位報錯 `Incorrect integer value`。改為安全的 `int()` 轉換並默認 0

---

## [v3.0.35] [2026-03-04] 課室日誌系統 — 完整功能實現

### 新增

- **課室日誌模組** — 教師掃碼課堂評級系統，支援 QR 碼掃描、課堂記錄提交、Review 查看、管理員 QR 碼生成與批量下載
- **管理員面板** — QR 碼網格、Reviewer 管理、班級列表
- **資料庫表** — `class_diary_entries`、`class_diary_reviewers`

---

## [v3.0.34] [2026-03-03] 學校學習中心知識圖譜輻射布局 + hover 高亮 + ICT 學習路徑

### 新增

- **輻射布局** — 學校學習中心知識圖譜改為輻射布局（Radial Layout）
- **hover 高亮** — 滑鼠懸停節點時高亮關聯節點和邊
- **ICT 學習路徑** — 新增 ICT 科目的學習路徑數據

---

## [v3.0.33] [2026-03-03] 修復 AI 對話頁總結按鈕缺失 + 統一工具欄樣式

### 修復

- **總結按鈕** — AI 對話頁工具欄缺少總結按鈕，補回並統一工具欄樣式

---

## [v3.0.32] [2026-03-03] 全球貿易大亨 — AI 庫存淺拷貝雙重計算 + 快速點擊防護

### 修復

- **AI 庫存計算** — 淺拷貝導致 AI 庫存被雙重計算，改為深拷貝
- **快速點擊防護** — 防止用戶快速連續點擊觸發重複操作

---

## [v3.0.31] [2026-03-03] 遊戲中心載入畫面對齊深綠主題風格

### 修改

- **載入畫面** — 遊戲中心載入動畫對齊其他頁面的深綠主題風格

---

## [v3.0.30] [2026-03-02] 全球貿易大亨遊戲集成 — 後端成績記錄 + 雙模式頁面

### 新增

- **全球貿易大亨** — 完整遊戲集成，支援後端成績記錄和雙模式（學生/遊客）頁面
- **TeacherDashboard** — 教師端成績查看 API 響應解析修復

### 修復

- **lucide-react polyfill** — 上傳遊戲缺少 lucide-react 圖標，添加完整 polyfill 防止 ReferenceError

---

## [v3.0.29] [2026-03-02] 簽到頁面新擬態表盤 + 數據庫結構文檔

### 新增

- **新擬態模擬表盤** — 簽到頁面新增 Neumorphism 風格模擬表盤

### 修復

- **統計數字不顯示** — 準時/遲到/嚴重遲到的統計數字不顯示

### 回退

- **時鐘恢復純數字** — 模擬表盤和倒影效果移除，恢復為純數字顯示

---

## [v3.0.28] [2026-02-28] 全站啟動畫面統一骨牌加載動畫

### 新增

- **骨牌動畫** — 全站啟動畫面統一為骨牌加載動畫風格
- **主界面啟動動畫** — 主界面新增啟動動畫

---

## [v3.0.27] [2026-02-28] 多項修復與 UI 增強

### 新增

- **ICT 知識圖譜** — 新增 ICT 科目知識圖譜數據

### 修復

- **學科顯示/刪除** — 修復學科相關的顯示和刪除問題
- **登錄小馬跟蹤** — 登錄頁小馬吉祥物跟蹤游標
- **聊天輸入欄** — 重新設計聊天輸入欄樣式

---

## [v3.0.26] [2026-02-27] 學校學習中心完整構建 — 獨立後端 + AI 問答 + PDF.js

### 新增

- **學校學習中心** — 全新模組，按科目和年級組織學習資源平台
- **獨立管理面板** — 學校學習中心新增獨立管理面板
- **內容感知 AI 問答** — 基於 PDF 頁碼引用和跳轉的 AI 問答
- **電子書式內嵌布局** — 教學資源改為電子書式內嵌布局
- **PDF.js 整合** — 用 PDF.js 替代 iframe 查看 PDF

### 修復

- **token 讀取** — 使用 `auth_token` 兼容登入系統
- **CSS/JS 路徑** — 修正學校學習中心靜態資源路徑
- **subjects 錯誤處理** — 加強錯誤處理並從 subjects 表拉取所有活躍學科
- **AI 助教頁碼跳轉** — 改用 PDF.js `goToPage`
- **流式問答頁碼** — 恢復 `page_references` 頁碼跳轉

### 重構

- **獨立後端** — 學校學習中心與 AI 學習中心完全分離

---

## [v3.0.25] [2026-02-27] AI / LLM 修復 — 動態 num_ctx + 思考分離 + 總結超時

### 修復

- **學習中心和課堂 AI** — 適配 `async_stream` 元組返回格式
- **總結超時** — invoke 改用 httpx + 動態 num_ctx，summary 關閉思考模式
- **思考過程顯示** — Ollama think 字段正確分離 thinking/answer

### 性能優化

- **動態 num_ctx** — 根據 prompt 長度自適應上下文窗口大小

---

## [v3.0.24] [2026-02-27] 全站 UI 風格升級 — Glassmorphism + Neumorphism

### 修改

- **全局 Glassmorphism** — 全站應用玻璃擬態設計風格
- **首頁 Glassmorphism** — 首頁專項玻璃擬態效果
- **登入頁 Neumorphism** — 登入頁面應用新擬態設計風格
- **點名系統 Neumorphism** — 點名系統應用新擬態設計風格
- **品牌 logo** — 移除首頁左上角「培僑中學」文字，僅保留 logo 圖片

### 修復

- **討論區帖子列表** — 修復中間帖子列表過窄問題

### 重構

- **登入頁獨立路由** — 從 index.html 中分離為獨立路由

---

## [v3.0.23] [2026-02-26] 學習總結功能修復 + 繁體化 + 管理後台角色區分

### 新增

- **管理後台角色區分** — 教師角色只顯示學科管理至學生分析模組

### 修復

- **學習總結模態框** — 修復模態框始終顯示，新增 CSS 樣式並設置初始隱藏
- **學習總結功能** — 修正 API 響應解包和狀態訪問路徑
- **文件上傳解析** — 修復解析問題並新增 `num_gpu` 配置支持
- **ai_chat URL** — 同步 `DEFAULT_APP_MODULES` 中 URL 為 `/chat`
- **學科名稱修改** — 前端字段名 `name` 改為 `subject_name`

### 修改

- **學習總結繁體化** — 學習總結模組全部簡體中文轉換為繁體中文
...
## [v3.0.22] [2026-02-26] 登入頁面企業級重構 — GSAP 動畫 + AI 小馬吉祥物

### 新增

- **AI 小馬吉祥物** — 登入頁面加入吉祥物，重新設計雙欄佈局
- **左右分屏佈局** — 登入頁面改為分屏佈局，啟動畫面絲滑過渡
- **GSAP Timeline** — 重寫啟動→登入過渡動畫
- **小馬滑動動畫** — 絲滑滑動效果 + 已登入用戶跳過啟動畫面
- **企業級重構** — 系統喚醒 + 玻璃面板空間展開效果

### 修改

- **品牌面板** — 左側品牌面板三段式垂直結構重構 + 布局微調
- **主頁吉祥物** — 移至標題左側，呼吸光感+搖擺動畫
- **點名系統動畫** — 深綠 GSAP 入場動畫

### 修復

- **啟動動畫閃爍** — 簡化過渡 + 品牌面板布局優化

---

## [v3.0.21] [2026-02-26] 遊戲中心修復 — 返回按鈕 + 管理按鈕 + 編輯頁代碼

### 修復

- **「我的遊戲」返回按鈕** — 改為跳轉遊戲中心而非主頁
- **遊戲編輯頁** — 加載原始代碼，跳過返回按鈕注入
- **管理按鈕** — 遊戲中心管理按鈕始終可見 + 上傳遊戲自動注入返回按鈕

---

## [v3.0.20] [2026-02-26] AI 學習中心 SSE 流式優化 + LLM 模型升級

### 新增

- **student_analysis_reports** — 為報表添加 `full_analysis_json` 欄位

### 修復

- **流式回答長度為 0** — 移除多餘的 ThinkingParser
- **課堂 AI 流式** — 移除多餘的 StreamingThinkingParser
- **對話標題保護** — num_ctx 提升至 128K + 深度思考開關修正
- **Ollama 500 錯誤** — num_ctx 納入配置體系 + 降至 32768 修復
- **對話標題覆蓋** — 修復標題被覆蓋及深度思考開關反轉
- **qwen3.5 回答死板** — 調整參數修復

### 性能優化

- **SSE 流式輸出** — AI 學習中心助教改為 SSE 流式，解決超時斷線
- **qwen3.5:35b 參數** — 適配官方推薦參數

### 修改

- **LLM 升級** — 本地模型從 qwen3:30b 升級為 qwen3.5:35b
- **System prompt** — 重寫 AI 學習伙伴 system prompt，提升回答質量

---

## [v3.0.19] [2026-02-25] Vision OCR 修復 + README 繁體化

### 修復

- **Vision OCR 圖形描述截斷** — thinking JSON 提取 + 嵌套物件正則 + 容錯解析
